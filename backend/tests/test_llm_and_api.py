...
def test_parse_new_schema_fields():
    raw = (
        '{"stance":"oppose","rationale":"r","blind_spot":"b",'
        '"blind_spot_reason":"job schedule blocks applications",'
        '"affected_group":"older workers","grounding":"inferred",'
        '"reframing":"make it easier to apply",'
        '"expected_complaint":"where do I apply?",'
        '"stance_strength":"legacy","caveat":"legacy","persona_link":{"direct":"legacy"}}'
    )

    parsed = parse_agent_response(raw)

    assert parsed["blind_spot_reason"] == "job schedule blocks applications"
    assert parsed["grounding"] == "inferred"
    assert "stance_strength" not in parsed
    assert "caveat" not in parsed
    assert "persona_link" not in parsed
...
def test_parse_agent_response_keeps_openai_only_fields_for_openai():
    parsed = parse_agent_response(
        '{"stance":"neutral","rationale":"depends",'
        '"blind_spot":"application hours miss shift workers",'
        '"blind_spot_reason":"professional_persona says night shift",'
        '"affected_group":"shift workers",'
        '"grounding":"direct",'
        '"reframing":"make access asynchronous",'
        '"persona_link":{"direct":"legacy","inferred":"legacy"}}'
    )

    assert parsed["stance"] == "neutral"
    assert parsed["reframing"] == "make access asynchronous"
    assert parsed["blind_spot_reason"] == "professional_persona says night shift"
    assert parsed["grounding"] == "direct"
    assert "persona_link" not in parsed
    return
...
def test_parse_agent_response_keeps_caveat_and_stance_strength():
    parsed = parse_agent_response(
        '{"stance":"support","stance_strength":"leaning","rationale":"ok",'
        '"caveat":"needs safeguards"}'
    )
...
def test_agent_messages_explain_general_stance_and_blind_spot_rules():
    messages = build_agent_messages({"agent_id": 1}, "policy", model_provider="openai")
    full_prompt = "\n".join(message["content"] for message in messages)

    assert "blind_spot" in full_prompt
    assert "blind_spot_reason" in full_prompt
    assert "grounding" in full_prompt
    assert "caveat" not in full_prompt
    assert "persona_link" not in full_prompt
    return
...
def test_agent_messages_limit_policy_expert_style_and_caveat_scope():
    messages = build_agent_messages(
        {
            ...
            "narrative_context": {"persona": "small shop owner"},
        },
        "policy",
        model_provider="openai",
    )
    ...
    assert "grounding rules" in full_prompt
    assert "blind_spot_reason" in full_prompt
    assert "caveat" not in full_prompt
    return
...
def test_structure_policy_uses_control_model(monkeypatch):
    from app.services import llm_client

    seen = {}

    def fake_raw(policy_text, model):
        seen["policy_text"] = policy_text
        seen["model"] = model
        return '{"policy_name":{"value":"p","source":"stated"},"target":null,"apply_method":null,"exclusions":null,"context":null}'

    monkeypatch.setattr(llm_client, "_structure_policy_raw", fake_raw)

    out = llm_client.structure_policy("policy text", model="gpt-5.5")

    assert out["policy_name"]["value"] == "p"
    assert seen == {"policy_text": "policy text", "model": "gpt-5.5"}


def test_structure_policy_raw_uses_responses_api_for_control_model(monkeypatch):
    import openai
    from app.services import llm_client
    ...
    class CapturingResponses:
        def create(self, **kwargs):
            ...
            return SimpleNamespace(output_text='{"policy_name":{"value":"p","source":"stated"}}')
    ...
    raw = llm_client._structure_policy_raw("policy text", model="gpt-5.5")

    assert raw == '{"policy_name":{"value":"p","source":"stated"}}'
...
def test_simulate_request_defaults_to_openai_model_and_accepts_options():
    ...
    assert req.control_model == "gpt-5.5"
    assert req.thinking is True
    assert req.persona_depth == "full"


def test_simulate_request_rejects_blank_control_model():
    with pytest.raises(ValidationError):
        SimulateRequest(policy="policy", control_model=" ")
...
def test_response_event_includes_8_axis_strata_and_new_schema():
    from app.api.simulate import response_event_from_result
    ...
    result = {
        "stance": "oppose",
        "rationale": "r",
        "expected_complaint": "where",
        "blind_spot": "b",
        "blind_spot_reason": "because",
        "affected_group": "g",
        "grounding": "direct",
        "reframing": "frame",
    }
    ...
    assert ev["occupation_stratum"] == "3"
    assert ev["household_stratum"] == "부부+자녀"
    assert ev["housing_stratum"] == "아파트"
    assert ev["education_stratum"] == "4년제 대학교"
    assert ev["field_stratum"] == "경영·행정·법"
    assert ev["blind_spot_reason"] == "because"
    assert ev["grounding"] == "direct"
    assert "stance_strength" not in ev
    assert "caveat" not in ev
    assert "persona_link" not in ev
...
def patch_fast_simulation(monkeypatch, simulate_api, agent_stream=None, discovery_summary=None):
    ...
    monkeypatch.setattr(
        simulate_api,
        "summarize_discovery",
        lambda aggregate, responses, model: discovery_summary
        or {
            "merged_blind_spots": [],
            "merged_reframings": [],
            "merged_complaints": [],
...
def test_simulate_stream_event_order_with_discovery_events(monkeypatch):
    from app.api import simulate as simulate_api
    ...
    assert "summary_prompt" not in events
    assert "summary_status" not in events
    assert "summary_token" not in events
    # discovery 단계: aggregate → prompt → status(started) → summary → status(completed) → done
    assert events.index("discovery_aggregate") < events.index("discovery_summary_prompt") < events.index("discovery_summary")
    assert "discovery_summary_status" in events
    assert events[-1] == "done"
    assert events[-2] == "discovery_summary_status"
    assert "discovery_summary" in events
...
def test_simulate_stream_emits_discovery_aggregate_and_summary(monkeypatch):
    from app.api import simulate as simulate_api
        ...
        **kwargs,
        ...
        yield {
            ...
            "response": {
                ...
                "blind_spot_reason": "work schedule",
                "affected_group": "shift workers",
                "grounding": "direct",
                "reframing": "make it async",
                "expected_complaint": "where do I apply?",
            },
        }
    ...
    discovery_summary = {
        "merged_blind_spots": [
            {"label": "access", "text": "application access", "agent_ids": [0, 1], "grounding": "direct"}
        ],
        "merged_reframings": [{"label": "async", "agent_ids": [0]}],
        "merged_complaints": [{"label": "where apply", "agent_ids": [0]}],
        "featured_axis": {"primary": "occupation_stratum", "secondary": None},
        "featured_axis_rationale": "Occupation exposes access barriers.",
    }

    patch_fast_simulation(monkeypatch, simulate_api, agent_stream=agent_stream, discovery_summary=discovery_summary)

    client = TestClient(app)
    response = client.post("/api/simulate", json={"policy": "policy", "n_agents": 5})
    events = _collect_sse(response.text)

    aggregate = next(event["data"] for event in events if event["type"] == "discovery_aggregate")
    summary = next(event["data"] for event in events if event["type"] == "discovery_summary")

    assert aggregate["axes"]["occupation_stratum"]
    assert "rate" not in json.dumps(aggregate, ensure_ascii=False)
    assert summary["merged_blind_spots"][0]["agent_ids"] == [0, 1]
    assert summary["featured_axis"] == discovery_summary["featured_axis"]


def test_simulate_stream_emits_discovery_aggregate_when_summary_fails(monkeypatch):
    from app.api import simulate as simulate_api

    patch_fast_simulation(monkeypatch, simulate_api)
    monkeypatch.setattr(simulate_api, "summarize_discovery", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("boom")))

    client = TestClient(app)
    response = client.post("/api/simulate", json={"policy": "policy", "n_agents": 5})
    event_types = [line.removeprefix("event: ") for line in response.text.splitlines() if line.startswith("event: ")]

    assert "discovery_aggregate" in event_types
    assert "discovery_summary" in event_types
