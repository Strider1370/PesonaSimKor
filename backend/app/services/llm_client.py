...
stance rules:
Choose support when the persona broadly accepts the policy direction.
Choose oppose when the persona broadly rejects the policy direction.
Choose neutral only when the persona cannot choose either direction.
...
def _structure_policy_raw(policy_text: str, model: str = DEFAULT_OPENAI_MODEL) -> str:
    from openai import OpenAI

    import httpx
    client = OpenAI(api_key=get_openai_api_key(), http_client=httpx.Client(verify=False))
...
def structure_policy(policy_text: str, model: str = DEFAULT_OPENAI_MODEL) -> dict:
    try:
        try:
            raw = _structure_policy_raw(policy_text, model)
        except TypeError:
            raw = _structure_policy_raw(policy_text)
...
def parse_agent_response(text: str, model_provider: str = "openai") -> dict:
    ...
    for field in ("expected_complaint",):
    ...
    blind_spot_reason = clean_optional_text(parsed.get("blind_spot_reason"))
    if blind_spot:
        result["blind_spot"] = blind_spot
        if blind_spot_reason:
            result["blind_spot_reason"] = blind_spot_reason
        if affected_group:
            result["affected_group"] = affected_group
        grounding = clean_optional_text(parsed.get("grounding"))
        if grounding in {"direct", "inferred"}:
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "Merge blind spots that share the same policy bottleneck or administrative failure mode, "
                    "even when affected_group wording differs. Prefer 4 to 7 high-signal blind_spot_clusters and "
                    "affected_group_clusters when many one-person items repeat the same root cause. "
