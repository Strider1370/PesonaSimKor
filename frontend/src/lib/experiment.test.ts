...
describe("experiment slots", () => {
  ...
  it("builds snapshot results from discovery outputs", () => {
    const results = buildSnapshotResults(
      ...
      ],
      {
        A: {
          responses: [
            { agent_id: 1, stance: "support" },
            { agent_id: 2, stance: "oppose" },
          ] as any,
          discoveryAggregate: {
            axes: { occupation_stratum: { "3": { presence: true, blind_spot_headcount: 1, agent_ids: [2], category_population: 3 } } },
          ...
          discoverySummary: {
            merged_blind_spots: [{ label: "access", text: "access", agent_ids: [2], grounding: "direct" }],
            merged_reframings: [],
            merged_complaints: [],
    ...
    expect(results[0]).toMatchObject({
      slotId: "A",
      presetId: "preset-a",
      total: { support: 1, oppose: 1, neutral: 0 },
      stability: null,
      realOpinion: null,
      sampledAgents: [],
      responses: [
  ...
  it("restores saved snapshot details into run state", () => {
    const runs = restoreSnapshotRuns([
      {
        ...
        llmOutputs: { 3: "raw" },
        discoveryAggregate: {
          axes: { occupation_stratum: { workers: { presence: true, blind_spot_headcount: 1, agent_ids: [3], category_population: 1 } } },
          featured_axis: { primary: "occupation_stratum", secondary: null },
        },
        discoveryAggregateRuns: [
          {
    ...
    expect(runs.A?.discoveryAggregate?.featured_axis.primary).toBe("occupation_stratum")
    expect(runs.A?.discoveryAggregateRuns).toHaveLength(1)
  })

  it("restores old summary-only snapshots without crashing or fabricating discovery outputs", () => {
    const runs = restoreSnapshotRuns([
      {
        slotId: "A",
        presetId: "preset-a",
    ...
    expect(runs.A?.discoveryAggregate).toBeNull()
    expect(runs.A?.discoveryAggregateRuns).toEqual([])
