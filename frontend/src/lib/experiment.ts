...
export function buildSnapshotResults(
  ...
  return slots.flatMap((slot) => {
    ...
    if (!run?.discoveryAggregate) return []
    const responses = run.responses ?? []
    const total = countsFromResponses(responses)
    return [
      {
        slotId: slot.id,
        ...
        discoveryAggregateRuns: run.discoveryAggregateRuns ?? [],
        discoverySummary: run.discoverySummary ?? null,
...
export function restoreSnapshotRuns(results: ExperimentSnapshotResult[]): Partial<Record<PolicySlotId, RestoredSnapshotRun>> {
  return Object.fromEntries(
    results.map((result) => {
      const discoveryAggregate = result.discoveryAggregate ?? null
      const discoveryAggregateRuns = result.discoveryAggregateRuns ?? []
      return [
        result.slotId,
        {
          phase: discoveryAggregate ? "done" : "idle",
          ...
          currentRunIndex: Math.max(0, discoveryAggregateRuns.length - 1),
          error: null,
        },
      ]
    }),
...
function countsFromResponses(responses: AgentRespondedEvent[]): MinimalAggregate["total"] {
